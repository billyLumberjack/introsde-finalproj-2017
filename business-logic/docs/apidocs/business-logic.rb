...
module Model
  ...
  class Statistics 
    ...
    # the json hash for this Statistics
    def to_jaxb_json_hash
      _h = {}
      _h['spent'] = spent.to_jaxb_json_hash unless spent.nil?
      _h['introduced'] = introduced.to_jaxb_json_hash unless introduced.nil?
      _h['difference'] = difference.to_jaxb_json_hash unless difference.nil?
      return _h
    end

    # the json (string form) for this Statistics
    def to_json
      to_jaxb_json_hash.to_json
    end

    #initializes this Statistics with a json hash
    def init_jaxb_json_hash(_o)
        if !_o['spent'].nil?
          _oa = _o['spent']
            if(_oa.is_a? Hash)
              @spent = EnunciateHelpers::LAMB_CLASS_AWARE.call(_oa) if _oa['@class']
              @spent =  Fixnum.from_json(_oa) unless _oa['@class']
            elsif (_oa.is_a? Array)
              #an array(of hashes hopefully) or scalar
              @spent = Array.new
              _oa.each { | _item | 
                 if ((_item.nil? || _item['@class'].nil?)rescue true)
                   @spent.push Fixnum.from_json(_item)
                 else
                   @spent.push EnunciateHelpers::LAMB_CLASS_AWARE.call(_item)
                 end
               }
            else
                @spent = _oa
        ...
        if !_o['introduced'].nil?
          _oa = _o['introduced']
            if(_oa.is_a? Hash)
              @introduced = EnunciateHelpers::LAMB_CLASS_AWARE.call(_oa) if _oa['@class']
              @introduced =  Fixnum.from_json(_oa) unless _oa['@class']
            elsif (_oa.is_a? Array)
              #an array(of hashes hopefully) or scalar
              @introduced = Array.new
              _oa.each { | _item | 
                 if ((_item.nil? || _item['@class'].nil?)rescue true)
                   @introduced.push Fixnum.from_json(_item)
                 else
                   @introduced.push EnunciateHelpers::LAMB_CLASS_AWARE.call(_item)
                 end
               }
            else
                @introduced = _oa
        ...
        if !_o['difference'].nil?
          _oa = _o['difference']
            if(_oa.is_a? Hash)
              @difference = EnunciateHelpers::LAMB_CLASS_AWARE.call(_oa) if _oa['@class']
              @difference =  Fixnum.from_json(_oa) unless _oa['@class']
            elsif (_oa.is_a? Array)
              #an array(of hashes hopefully) or scalar
              @difference = Array.new
              _oa.each { | _item | 
                 if ((_item.nil? || _item['@class'].nil?)rescue true)
                   @difference.push Fixnum.from_json(_item)
                 else
                   @difference.push EnunciateHelpers::LAMB_CLASS_AWARE.call(_item)
                 end
               }
            else
                @difference = _oa
    ...
    # constructs a Statistics from a (parsed) JSON hash
    def self.from_json(o)
      if o.nil?
        return nil
      else
